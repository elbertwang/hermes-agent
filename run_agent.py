            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        # with open models.  Nudge up to 3 times before
                        # giving up.  Skip when content has inline
                        # <think> tags (model chose to reason, just no
                        # visible text — different from truly empty).
                        ...
                        if _truly_empty and not _has_structured and self._empty_content_retries < 3:
                            ...
                            self._vprint(
                                ...
                                f"— nudging model ({self._empty_content_retries}/3)",
                                force=True,
                            )
                            interim_msg = self._build_assistant_message(
                                assistant_message, "incomplete"
